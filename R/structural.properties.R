...
get.diameter <- function(graph , directed = TRUE , unconnected = TRUE , weights = NULL) {
   lifecycle::deprecate_soft("1.5.0", "get.diameter()", "get_diameter()")
...
farthest.nodes <- function(graph , directed = TRUE , unconnected = TRUE , weights = NULL) {
   lifecycle::deprecate_soft("1.5.0", "farthest.nodes()", "farthest_vertices()")
...
degree.distribution <- function(graph , cumulative = FALSE , ...) {
   lifecycle::deprecate_soft("1.5.0", "degree.distribution()", "degree_distribution()")
...
shortest.paths <- function(graph , v = V(graph) , to = V(graph) , mode = c("all","out","in") , weights = NULL , algorithm = c("automatic","unweighted","dijkstra","bellman-ford","johnson")) {
   lifecycle::deprecate_soft("1.5.0", "shortest.paths()", "distances()")
...
get.shortest.paths <- function(graph , from , to = V(graph) , mode = c("out","all","in") , weights = NULL , output = c("vpath","epath","both") , predecessors = FALSE , inbound.edges = FALSE , algorithm = c("automatic","unweighted","dijkstra","bellman-ford")) {
   lifecycle::deprecate_soft("1.5.0", "get.shortest.paths()", "shortest_paths()")
   shortest_paths(graph = graph, from = from, to = to, mode = mode, weights = weights, output = output, predecessors = predecessors, inbound.edges = inbound.edges, algorithm = algorithm, x = x, x = x)
}
...
get.all.shortest.paths <- function(graph , from , to = V(graph) , mode = c("out","all","in") , weights = NULL) {
   lifecycle::deprecate_soft("1.5.0", "get.all.shortest.paths()", "all_shortest_paths()")
...
induced.subgraph <- function(graph , vids , impl = c("auto","copy_and_delete","create_from_scratch")) {
   lifecycle::deprecate_soft("1.5.0", "induced.subgraph()", "induced_subgraph()")
...
neighborhood.size <- function(graph , order = 1 , nodes = V(graph) , mode = c("all","out","in") , mindist = 0) {
   lifecycle::deprecate_soft("1.5.0", "neighborhood.size()", "ego_size()")
...
#' `r lifecycle::badge("deprecated")`
#'
#' `neighborhood()` was renamed to `ego()` to create a more
#' consistent API.
#' @inheritParams ego
#' @keywords internal
#' @export
neighborhood <- function(graph , order = 1 , nodes = V(graph) , mode = c("all","out","in") , mindist = 0) {
   lifecycle::deprecate_soft("1.5.0", "neighborhood()", "ego()")
   ego(graph = graph, order = order, nodes = nodes, mode = mode, mindist = mindist, x = x)
}
...
graph.neighborhood <- function(graph , order = 1 , nodes = V(graph) , mode = c("all","out","in") , mindist = 0) {
   lifecycle::deprecate_soft("1.5.0", "graph.neighborhood()", "make_ego_graph()")
...
clusters <- function(graph , mode = c("weak","strong")) {
   lifecycle::deprecate_soft("1.5.0", "clusters()", "components()")
...
is.maximal.matching <- function(graph , matching , types = NULL) {
   lifecycle::deprecate_soft("1.5.0", "is.maximal.matching()", "is_max_matching()")
...
maximum.bipartite.matching <- function(graph , types = NULL , weights = NULL , eps = .Machine$double.eps) {
   lifecycle::deprecate_soft("1.5.0", "maximum.bipartite.matching()", "max_bipartite_match()")

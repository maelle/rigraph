...
edge.betweenness <- function(graph , e = E(graph) , directed = TRUE , weights = NULL , cutoff = -1) {
   lifecycle::deprecate_soft("1.5.0", "edge.betweenness()", "edge_betweenness()")
...
#' `r lifecycle::badge("deprecated")`
#'
#' `edge.betweenness.estimate()` was renamed to `estimate_edge_betweenness()` to create a more
#' consistent API.
#' @inheritParams estimate_edge_betweenness
#' @keywords internal
#' @export
edge.betweenness.estimate <- function(graph , e = E(graph) , directed = TRUE , cutoff , weights = NULL) {
   lifecycle::deprecate_soft("1.5.0", "edge.betweenness.estimate()", "estimate_edge_betweenness()")
   estimate_edge_betweenness(graph = graph, e = e, directed = directed, cutoff = cutoff, weights = weights)
}
...
#' `r lifecycle::badge("deprecated")`
#'
#' `closeness.estimate()` was renamed to `estimate_closeness()` to create a more
#' consistent API.
#' @inheritParams estimate_closeness
#' @keywords internal
#' @export
closeness.estimate <- function(graph , vids = V(graph) , mode = c("out","in","all","total") , cutoff , weights = NULL , normalized = FALSE) {
   lifecycle::deprecate_soft("1.5.0", "closeness.estimate()", "estimate_closeness()")
   estimate_closeness(graph = graph, vids = vids, mode = mode, cutoff = cutoff, weights = weights, normalized = normalized)
}

...
#' `r lifecycle::badge("deprecated")`
#'
#' `edge.disjoint.paths()` was renamed to `edge_disjoint_paths()` to create a more
#' consistent API.
#' @inheritParams edge_disjoint_paths
#' @keywords internal
#' @export
edge.disjoint.paths <- function(graph , source , target) {
   lifecycle::deprecate_soft("1.5.0", "edge.disjoint.paths()", "edge_disjoint_paths()")
   edge_disjoint_paths(graph = graph, source = source, target = target)
}
...
vertex.disjoint.paths <- function(graph , source = NULL , target = NULL) {
   lifecycle::deprecate_soft("1.5.0", "vertex.disjoint.paths()", "vertex_disjoint_paths()")
...
graph.adhesion <- function(graph , checks = TRUE) {
   lifecycle::deprecate_soft("1.5.0", "graph.adhesion()", "adhesion()")
